...
<template>
    <section class="workspace__wrapper">
        <a-spin :spinning="initLoading">
            <a-form class="form-write" ref="formRef" layout="vertical" :model="formModel" :rules="rules" :wrapper-col="{ span: 24 }">
                <a-form-item>
        ...
        <a-modal title="关联分类/标签" v-model:open="isRelationVisible" :footer="null" width="860px">
            <a-form class="form-releation" ref="relFormRef" layout="vertical" :model="relFormModel" :wrapper-col="{ span: 24 }">
                <a-form-item name="newTags" label="文章标签">
                    <a-form-item-rest>
                        <a-row :gutter="16" style="line-height: 2">
                ...
                <a-form-item name="categorys" label="已有分类">
                    <div>
                        ...
                        <br />
                        <a-form-item-rest>
                            <a-checkbox-group v-model:value="relFormModel.oldCategoryIds" style="width: 100%; margin-top: 8px">
                                <a-row :gutter="16">
...
const handleConfirmPublish = async () => {
    ...
    if (isEdit.value) {
        ...
        const deleteTagIDs = (articleDetail.value as ArticleDTO).tags.filter((tag) => !tagNames.includes(tag.tagName)).map((tag) => tag.id);
        ...
        const relatedCategoryIDs = relFormModel.oldCategoryIds.filter((categoryId) => !originalCategoryIds.value.includes(categoryId));
