...
export default defineComponent({
    ...
    setup() {
        ...
        const globalUIStateStore = useGlobalUIState();

        const { isMenuVisible } = storeToRefs(globalUIStateStore);

        const isAnimationEnabled = ref(false);

        const hideMenu = () => globalUIStateStore.setIsMenuVisible(false);
        ...
        const onToggleMenu = () => {
            ...
            if (isMenuVisible.value) {
                ...
                globalUIStateStore.setIsMenuVisible(true);
                // 在弹出菜单时保证禁用滚动
