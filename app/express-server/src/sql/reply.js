module.exports = {
    ...
    GetReplyOfMsgWaitReview:
        "SELECT r1.*, c1.content AS comment_content, r2.content AS reply_to_content FROM reply r1\
    ...
    // 分页查询未审核的留言下的回复
    QueryUnreviewedMessageReplyPage:
        "SELECT SQL_CALC_FOUND_ROWS r1.*, c1.content AS comment_content, r2.content AS reply_to_content FROM reply r1\
        LEFT JOIN comments c1 ON r1.comment_id = c1.id\
        JOIN reply r2 ON r1.parent_id = r2.id\
        WHERE (r1.approved = 0 AND r1.parent_id IS NOT NULL AND c1.article_id IS NULL)\
    UNION\
    ...
    GetReplyOfCommentWaitReview:
        "SELECT r1.*, c1.content AS comment_content, r2.content AS reply_to_content, a1.article_name FROM reply r1\
        LEFT JOIN comments c1 ON r1.comment_id = c1.id\
    ...
    // 分页查询未审核的评论下的回复
    QueryUnreviewedCommentReplyPage:
        "SELECT SQL_CALC_FOUND_ROWS r1.*, c1.content AS comment_content, r2.content AS reply_to_content, a1.article_name FROM reply r1\
        LEFT JOIN comments c1 ON r1.comment_id = c1.id\
        LEFT JOIN article a1 ON a1.id = c1.article_id\
        JOIN reply r2 ON r1.parent_id = r2.id\
        WHERE (r1.approved = 0 AND r1.parent_id IS NOT NULL AND c1.article_id IS NOT NULL)\
    ...
    QueryReplyByCommentID:
        "SELECT a.id, a.comment_id, a.content, a.create_time, a.nick_name, a.parent_id, a.site_url, a.device, a.avatar, b.nick_name AS reply_name FROM reply a\
