module.exports = {
    ...
    GetArticleByID: 'SELECT a.*, u.nick_name AS author, GROUP_CONCAT(DISTINCT c.id SEPARATOR " ") AS categoryIDs, GROUP_CONCAT(DISTINCT c.category_name SEPARATOR " ") AS categoryNames, GROUP_CONCAT(DISTINCT t.id SEPARATOR " ") AS tagIDs, GROUP_CONCAT(DISTINCT t.tag_name SEPARATOR " ") AS tagNames FROM article a\
        LEFT JOIN user u ON a.author_id = u.id\
        LEFT JOIN article_category a_c ON a.id = a_c.article_id\
        LEFT JOIN category c ON a_c.category_id = c.id\
        LEFT JOIN article_tag a_t ON a.id = a_t.article_id\
    ...
    GetPagedArticleByCategory: 'SELECT SQL_CALC_FOUND_ROWS a.id, a.article_name, a.poster, a.read_num, a.summary, a.create_time, a.update_time, u.nick_name AS author, GROUP_CONCAT(DISTINCT c.id SEPARATOR " ") AS categoryIDs, GROUP_CONCAT(DISTINCT c.category_name SEPARATOR " ") AS categoryNames, GROUP_CONCAT(DISTINCT t.id SEPARATOR " ") AS tagIDs, GROUP_CONCAT(DISTINCT t.tag_name SEPARATOR " ") AS tagNames FROM article a\
        LEFT JOIN user u ON a.author_id = u.id\
        LEFT JOIN article_category a_c ON a.id = a_c.article_id\
        LEFT JOIN category c ON a_c.category_id = c.id\
        LEFT JOIN article_tag a_t ON a.id = a_t.article_id\
    ...
    QueryPreAndNextArticleIds: 'SELECT id, article_name FROM article WHERE id in ((SELECT id FROM article WHERE private = 0 AND deleted = 0 AND id < ? ORDER BY id DESC limit 1), (SELECT id FROM article WHERE private = 0 AND deleted = 0 AND id > ? ORDER BY id asc limit 1))',
    // 插入文章分类关系表
