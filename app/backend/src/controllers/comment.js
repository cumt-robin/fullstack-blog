...
router.post("/add", (req, res, next) => {
    const params = { ...req.body, create_time: new Date() };
    // XSS防护
    if (params.content) {
 ...
 */
router.get("/get_not_approved", (req, res, next) => {
    const sqlStr = Number(req.query.type) === 1 ? indexSQL.QueryCommentsNotApproved : indexSQL.QueryMessagesNotApproved;
 ...
 */
router.get("/page_not_approved", (req, res, next) => {
    const params = req.query;
    const pageNo = Number(params.pageNo || 1);
    const pageSize = Number(params.pageSize || 10);
